...
mnist = data.load("cifar10")
test_data = mnist["test"].values()
train_data = mnist["train"]

net = Network(
    layers = [InputLayer([3,32,32]),
...
for i in range(200):
    for m in range(1000):
        net.train(*train_data.get_batch(30), learning_rate = 0.001 )
    ...
    #vis.vis_matrix(sample_xs[:2,0], s2x2 = True)
    ...
    for layer_name in sorted(full_info.keys()):
        ...
        if "ConvolutionLayer" in layer_name:
            ...
            for fn in [i%F]:
                ...
                vis.vis_matrix(info["y"][:2,fn,:22,:22], s2x2 = True)
                print 80*"-"
            ...
            C = info["W"].shape[1]
            vis.vis_matrix_signed(info["W"][:,i%C])
            ...
            vis.vis_matrix(np.repeat(info["y"], 2, axis = 0)[:30, :200], s2x2 = True)
