...
from ..utils import OuterDatasetDict, InnerDatasetDict
    ...
    def load(self, label_format = "index", flatvecs = False):
        ...
        for array_path in glob(path.join(self.path, "*_*.npy")):
            ...
            if set_name not in data:
                data[set_name] = InnerDatasetDict()
                ...
                if len(pres.shape) == 1 and pres.dtype in ["int", "int32", "int64"]:
                    if label_format == "vector":
                        ...
                        for n in xrange(len(pres)):
                            new_data[n][pres[n]] = 1
                        pres = new_data
                if len(pres.shape) == 3 and flatvecs:
                    N, X, Y = pres.shape
                    pres = pres.reshape(N, X*Y) 
