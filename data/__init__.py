...
def get_dataset_paths():
    ...
    if os.path.exists("datasets/"):
        paths.append(os.path.abspath("datasets/"))
    ...
    if not paths:
        ...
        if home:
            ...
            path = os.path.join(os.path.abspath("."), "datasets/")
        print "Assuming ", path
        print "Creating ", path
        os.mkdir(path)
        paths = [path]
...
def get_dataset_handle(name, path):
    handle_path = os.path.join(path, "handle.py")
    if os.path.exists(handle_path):
        handle = imp.load_source(name + ".handle", handle_path )
    else:
        #Fallback:
        from helpers.NumpyImageLabelSet import NumpyImageLabelSet
        handle = NumpyImageLabelSet(handle_path)
    return handle
